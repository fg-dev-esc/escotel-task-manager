...
import { eliminarTarea } from '../../../services/tasks'
import dayjs from 'dayjs'

const { Text } = Typography

export default function TaskList({ tareas, onEditTask, onRecargar }) {
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (_, record) => (
        <Space>
          <Button type="text" icon={<EditOutlined />} onClick={() => onEditTask(record)} />
  ...
  return (
    <Table
      ...
      columns={columns}
      onRow={(record) => ({ onClick: () => onEditTask(record), style: { cursor: 'pointer' } })}
